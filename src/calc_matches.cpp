...
double calc_matches (const Species& species1, const Species& species2, const int& weight, const int& dc,
        ...
        for (unsigned int i = 0; i < spacedWords1.size(); ++i) {
            ...
            if (bl1 > 1) {
                ...
                if (best[0] >= threshold) {
                    ...
                    total_dc += dc;
                    if (outputScores) {
                        auto exists = matchCount.find(dc - best[1]);
                        if (exists != matchCount.end() ) {
                            matchCount[dc - best[1]] += 1;
                        }
                        else {
                            matchCount.insert(pair<int, int>(dc - best[1], 1));
                        }
                    }
                }
                if (bl1 > 1 && multi_done) {
                    ...
                    if (bl2 > 1) {
                        ...
                        if (spacedWords1[i].key == spacedWords2[j].key) {
                            ...
                            if (score >= threshold) {
                                ...
                                total_dc += dc;
                                if (outputScores) {
                                    auto exists = matchCount.find(dc - mismatches);
                                    if (exists != matchCount.end() ) {
                                        matchCount[dc - mismatches] += 1;
                                    }
                                    else {
                                        matchCount.insert(pair<int, int>(dc - mismatches, 1));
                                    }
                                }
                            }
                            break;
    ...
    if (outputScores) {
        scoreOutput(matchCount, species1.header, species2.header);
    }
